local solidvar = CreateClientConVar("cl_solid_mode", "1", true, true, "Solid Mode.  Set to 0 to stop yourself from colliding with anything." )

local function PlayerNonSolid(ent1,ent2)
	if (ent1:IsPlayer() && ent1:GetInfoNum("cl_solid_mode",1) == 0) || (ent2:IsPlayer() && ent2:GetInfoNum("cl_solid_mode",1) == 0) then return false end
end
