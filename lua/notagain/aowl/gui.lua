...
if CLIENT then
    ...
    aowlgui.Close = function()
        frame:Hide()
    end

    aowlgui.IsOpened = function()
        return IsValid(frame) and frame:IsVisible()
    end
    ...
    aowlgui.LogCommand = function(ply,cmd,args)
    ...
    aowlgui.UpdateCmdList = function(search)
        list:Clear()
        if not search then
    ...
    aowlgui.UpdateAliases = function(cmd)
        ...
        for k,v in pairs(aliases) do
            if v ~= cmd then
                local lbl = setup:Add("DLabel")
                lbl:SetText("⮞ "..v)
                lbl:SetTextColor(Color(30,30,30))
                lbl:SetPos(95+lsearch:GetWide(),-10+(10*i))
                lbl:SetSize(150,20)
    ...
    updatecmds.DoClick = RequestCommands
