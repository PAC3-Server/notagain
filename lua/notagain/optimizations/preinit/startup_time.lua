...
local ok = CreateConVar("profile_startup", "0", FCVAR_ARCHIVE)
...
if ok:GetBool() then
	...
	local FORMAT_HEADER_LINE       = "| %-90s: %-40s: %-20s: %-12s: %-12s: %-12s|\n"
	...
	local FORMAT_TITLE             = "%-90.90s: %-40.40s: %-20s"
	...
	function ProFi:writeReportsToFilename( filename )
		local file, err = file.Open( filename, 'w', "DATA" )
		assert( file, err )
...
local function dump()
	ProFi:writeReport( "startup_time_"..(CLIENT and "cl" or "sv")..".txt" )
end

if CLIENT then
	timer.Create("profile_startup", 0, 0, function()
		local p = LocalPlayer()
		if p:IsValid() then
			if ok:GetBool() then
				ProFi:stop()
			...
			print("startup took " .. (SysTime() - start_time) .. " seconds")
			timer.Remove("profile_startup")
		end
	end)
end
