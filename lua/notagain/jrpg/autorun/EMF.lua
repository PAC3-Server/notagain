...
if SERVER then
	...
	function EMF.AddTopology( pos )
		...
		for index , topo in pairs( EMF.Topology ) do
			if !EMF.Topology[index].CreatedOnInit and topo.Pos:Distance( pos ) < EMF.MaxDistToRef / 2 then
				add = false
				break
			end
		end
	...
	function EMF.IsStuck( ent )
		...
		local refpos  = ent:WorldSpaceCenter() - ent:NearestPoint( ent:GetPos() - Vector( 0 , 0 , BigValue ) )
