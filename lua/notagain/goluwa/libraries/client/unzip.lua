...
local function BitReader(f)
	...
	function self:ReadBits(nbits)
		...
		for i = 0, nbits, 8 do
			...
			self.buf_byte = self.buf_byte + bit.lshift(f:ReadByte() or 0, self.buf_nbit)
...
local function unzip(path, where, validate)
	local zip = file.Open(path, "rb", where or "DATA")
	...
	while true do
		...
		data.compressed_size = zip:ReadLong()
		data.uncompressed_size = zip:ReadLong()
		data.filename_length = zip:ReadShort()
		data.extra_field_length = zip:ReadShort()
		data.file_name = zip:Read(data.filename_length)
		zip:Seek(zip:Tell() + data.extra_field_length)
	...
	if validate then
		...
		for _, data in ipairs(files) do
			if data.file_content then
				if util.CRC(data.file_content) ~= tostring(data.crc) then
					errors = errors .. data.file_name:match(".-/(.+)") .. "crc ("..util.CRC(data.file_content)..") does not match "..data.crc.."!\n"
				end
			end
		end
		if errors ~= "" then
			print(errors)
			error("archive crc validation failed!")
		end
	end
