...
function cutf8.byte(char, offset)
...
function cutf8.bytelength(char, offset)
...
function cutf8.char(byte)
	local cutf8 = ""

	if byte <= 127 then
		cutf8 = string.char(byte)
...
function cutf8.sub(str, i, j)
	j = j or -1

	local length = 0

	-- only set l if i or j is negative
	local l = (i >= 0 and j >= 0) or cutf8.length(str)
	...
	for _ = 1, bytes do
		...
		pos = pos + cutf8.bytelength(str, pos)
...
local function cutf8replace(str, mapping)
	local pos = 1
	local bytes = str:len()
	local char_bytes
	local new_str = ""
	...
	while pos <= bytes do
		char_bytes = cutf8.bytelength(str, pos)
...
local upper, lower = table.GetKeys(cutf8.data),cutf8.data

function cutf8.upper(str)
	return cutf8replace(str, upper)
end

function cutf8.lower(str)
	return cutf8replace(str, lower)
end

function cutf8.length(str)
	local len = 0
	for i = 1, #str do
		local b = str:byte(i)
...
for name, func in pairs(cutf8) do
	string["u" .. name] = func
end
