-- this doesn't do much but it's mostly to reduce the amount of hook noise for profiling

local cvars = {
	pp_bokeh = {{"RenderScreenspaceEffects", "RenderBokeh"}, {"NeedsDepthPass", "NeedsDepthPass_Bokeh"}},
	pp_bloom = {"RenderScreenspaceEffects", "RenderBloom"},
	pp_color = {{"RenderScreenspaceEffects", "RenderColorModify"}},
	pp_dof = {{"Think", "DOFThink"}},
	pp_fb = {{"PostRender", "RenderFrameBlend"}},
	pp_motionblur = {{"RenderScreenspaceEffects", "RenderMotionBlur"}},
	pp_mat_overlay = {{"RenderScreenspaceEffects", "RenderMaterialOverlay"}}, -- exception
	pp_sharpen = {{"RenderScreenspaceEffects", "RenderSharpen"}},
	pp_sobel = {{"RenderScreenspaceEffects", "RenderSobel"}},
	pp_stereoscopy = {{"RenderScene", "RenderStereoscopy"}},
	pp_sunbeams = {{"RenderScreenspaceEffects", "RenderSunbeams"}},
	pp_texturize = {{"RenderScreenspaceEffects", "RenderTexturize"}},
	pp_toytown = {{"RenderScreenspaceEffects", "RenderToyTown"}}
}

timer.Simple(0, function()
	local hook_table = hook.GetTable()
	for cvar_name, hooks in pairs(cvars) do
		for _, data in ipairs(hooks) do
			if hook_table[data[1]] and hook_table[data[1]][data[2]] then
				data[3] = hook_table[data[1]][data[2]] -- store the function
				hook.Remove(data[1], data[2])
			end
		end

		_G.cvars.AddChangeCallback(cvar_name, function(convar_name, value_old, value_new)
			if value_new ~= "0" and (cvar_name ~= "pp_mat_overlay" or value_new ~= "") then
				for _, data in ipairs(hooks) do
					hook.Add(data[1], data[2], data[3])
				end
			else
				for _, data in ipairs(hooks) do
					hook.Remove(data[1], data[2])
