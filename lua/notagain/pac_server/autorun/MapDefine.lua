	...
	--[[hook.Add("Move","OnChangedArea",function(ply,_) 
			...
			elseif #ply.Areas < areas then
				for area,_ in pairs(area) do
					if not ply.Areas[area] then
						hook.Run("OnAreaEntered",ply,area)
						break 
					end
				end
			end
	...
	if SERVER then 
		...
		util.AddNetworkString("MapSyncAreas")
	...
	if CLIENT then
		...
		net.Receive("MapSyncAreas",function()
