	...
	if want_restart or try_restart then
		...
		if afk then
			if not players[1] then
				if discordrelay and discordrelay.ready then discordrelay.notify("Auto Restart trigger: " .. (reason and reason or want_restart and "last restart was over 3 hours ago") or "???") end
			...
			if not restarting then
				...
				aowl.CountDown(15, "RESTARTING SERVER BECAUSE EVERYONE IS AFK", function()
					if discordrelay and discordrelay.ready then discordrelay.notify("Auto Restarting because everyone is afk " ..
						(reason and "and updates from " .. reason .. " are pending ..." or want_restart and "and last restart was over 3 hours ago")) end
					file.Write("server_last_restart.txt", tostring(os.time()))
					game.ConsoleCommand("changelevel " .. game.GetMap() .. "\n")
				end)
			end
